<template>
    ...
    <div class="wrap">
        ...
        <ul class="inpList for-main">
            ...
            <li class="wdI">{{$t('OperatingSystem.lists.MinorOperatingSystem.content1[0]')}}：</li>
            <li>
            ...
            <li class="wdI">{{$t('OperatingSystem.lists.MinorOperatingSystem.content1[1]')}}：</li>
            <li>
                <el-select v-model="options1" placeholder="" class="sel" disabled>
                    <el-option
                            v-for="item in options"
        ...
        <ul class="inpList for-main">
            ...
            <li class="wdI">{{$t('OperatingSystem.lists.MinorOperatingSystem.content2[0]')}}：</li>
            <li>
                <el-select v-model="options1" placeholder="" class="sel" disabled>
                    <el-option
                            v-for="item in options"
            ...
            <li class="wdI">{{$t('OperatingSystem.lists.MinorOperatingSystem.content2[1]')}}：</li>
            <li>
                <el-select v-model="options1" placeholder="" class="sel" disabled>
                    <el-option
                            v-for="item in options"
...
export default {
  ...
  methods: {
    async setrabbet() {
        let than=this
        const SQL = `   select cs.id,cs.software_desCN,cs.software_des,cs.years from component_software cs  join product_software ps on cs.id=ps.software_id
           where ps.software_id in (128,129,131,132) and ps.product_id=${localStorage.productId}`;
...
where temptable.component_id=component_software.id`
        const cpieK = ` select component_count from product_programme_detail where category_id=11 AND product_id='${localStorage.productId}' AND solution_id=${localStorage.solutionId} and template_id='${localStorage.templateId}'and machine_id='${localStorage.machineId}'`;// 获取cpu的激活数
        const SQLYear= `
...
select carry_software_id from software_software ss left join component_software cs on cs.id=ss.choice_software_id where choice_software_id=${this.options[this.value].id}
        ...
        if(this.id){
            ...
            ('${localStorage.productId}','${localStorage.machineId}','${this.options2[this.year].category_id}','${this.options2[this.year].childId}','${this.num}','${time}','${localStorage.solutionId}','${localStorage.templateId}')`;
            //添加三条数据
