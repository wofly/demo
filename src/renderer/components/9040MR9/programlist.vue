<template>
    ...
    <div class="wrap">
        <!--主要操作系统-->
        <div class="font-style content-label">{{$t('OperatingSystem.lists.MainOperatingSystem.ApplicationProgram')}} - PowerHA</div>
        ...
        <ul class="inpList for-main">
            ...
            <li v-show="value != 0" class="wdI">{{$t('OperatingSystem.lists.MainOperatingSystem.content1')}}：</li>
            <li v-show="value != 0">
            ...
            <li v-show="value != 0" class="wdI">{{$t('OperatingSystem.lists.MainOperatingSystem.content2')}}：</li>
            <li v-show="value != 0">
    ...
    export default {
        ...
        methods: {
            ...
            async setrabbet() {
                let than=this
                /*获取操作系统下拉数据*/
                const SQL = `select cs.id,cs.software_desCN,cs.software_des from component_software cs
join product_software ps on cs.id=ps.software_id
join product_info info on info.id=cs.productId
where info.machine_model='9040' and info.machine_submodel='MR9' and category_id=15 and cs.id=160`;
...
where cs.productId=2 and ss.choice_software_id =160 and years !='Null'  and years !=''
)   and category_id=20`
                ...
                await new Promise(function (resolve, reject) {
                    ...
                    console.log(this.options2,this.options,this.year)
                })
...
select carry_software_id from software_software ss left join component_software cs on cs.id=ss.choice_software_id where choice_software_id='${item.component_id}'
    ...
    where cs.productId=2 and ss.choice_software_id=${this.options[this.value].id}) and software_type=3`
                ...
                ('${localStorage.productId}','${localStorage.machineId}','15','${this.options[this.value].id}','${this.num}','${time}','${localStorage.solutionId}','${localStorage.templateId}'),
...
where cs.id in ( select component_id from product_programme_detail where seq_Key=${time} and template_id='${localStorage.templateId}' and solution_id='${localStorage.solutionId}'and product_id='${localStorage.productId}' and machine_id='${localStorage.machineId}')`;
                                    ...
                                    this.$db.all(selectSql, (err, res) => {
                                        ...
                                        res.forEach(item=>{
                                            ...
                                            if(item.category_id==20&&(item.software_FC==null || item.software_FC==undefined || item.software_FC=="")){
                                                this.fatherId=item.detail_id
                                            }
                                            if(item.category_id==20&&item.software_FC){
                                                this.childId=item.detail_id
                                            }
                                        })
                    ...
                    const del= `delete from product_programme_detail where id in(${this.id},${this.fatherId},${this.childId})`
                    console.log(del)
