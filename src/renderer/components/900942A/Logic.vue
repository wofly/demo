...
<script scoped>
    export default {
        ...
        methods: {
          async  getDatas() {
                /*逻辑分区数据获取*/
                const SQL_LJ = `SELECT*FROM component_software where id in(select carry_software_id from software_software where choice_software_id in(128,129,131,132)) and category_id =13`
                /*原始数据获取*/
                const SQL_FIRST = `select  cs.id,software_des , years,software_desCN       from component_software cs  join product_software ps on cs.id=ps.software_id

                where ps.software_id=133 and product_id=${localStorage.productId} `
              ...
              /*获取数据库更新*/
                const SQL_UP = `select detail.*,cs.id soft_id,cs.years,cs.software_FC from product_programme_detail detail ,component_software cs
                    ...
                    and  product_id='${localStorage.productId}'
                    and machine_id='${localStorage.machineId}'
                    and detail.category_id IN (13,21)
