...
export default {
  async exportCfrOrCsv(machineName, tailName, productId,solutionId, templateId, machineId) {
...
//==========================================数据源准备：从json数据源中获取的数据========================================start
  ...
  let cradDate = moment(dataListHW[0].ship_date)
      .format('YYYY-MM-DD');
  //循环硬件列表数据
  for (let v = 0; v < dataListHW.length + 1; v++) {
    ...
    if (flagHW) {
      ...
      setValueInJsonByKey(machinePropertyHW, 'LINE NO', getLineNo(1, v));
      setValueInJsonByKey(machinePropertyHW, 'MTM',
          machine_model + '-' + machine_submodel);
      setValueInJsonByKey(machinePropertyHW, 'DESCRIPTION', dataListHW[v].pdesc);
      ...
      setValueInJsonByKey(machinePropertyHW, 'LINE NO', getLineNo(1, v));
      setValueInJsonByKey(machinePropertyHW, 'FEATURE', dataListHW[v - 1].component_FC);
      setValueInJsonByKey(machinePropertyHW, 'DESCRIPTION', dataListHW[v - 1].description_en);
      setValueInJsonByKey(machinePropertyHW, 'QUANTITY', appendToString(dataListHW[v - 1].component_count));
      setValueInJsonByKey(machinePropertyHW, 'TYPE', 'ADD');
      //价格为0 也是可以定价的
      if (lookPrice == '1'){
        setValueInJsonByKey(machinePropertyHW, 'CHARGEABLE', 'N');
      }else{
        if (dataListHW[v - 1].listprice_onshore != undefined && dataListHW[v - 1].listprice_onshore != null && dataListHW[v - 1].listprice_onshore != '') {
          setValueInJsonByKey(machinePropertyHW, 'CHARGEABLE', 'Y');
        } else {
          setValueInJsonByKey(machinePropertyHW, 'CHARGEABLE', 'N');
        }
      ...
      if (headerSWMA[m].Classify == swmaList[o].Classify) {
        ...
        for (let k = 0; k < thirdSWMA.length; k++) {
          ...
          if (swmaList[o].id == thirdSWMA[k].choice_software_id) {
            ...
            setValueInJsonByKey(machinePropertySWMA2, 'QUANTITY', appendToString(thirdSWMA[k].component_count));
            num++;
