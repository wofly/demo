  ...
  <!--配置软件-->
    <div class="hardware">
        ...
        <div class="hardware-right">
           ...
           {{totalPrice | totalPriceFiltersSoftware}}
                        </span>
                        <span v-show="$store.state.userId ==1">- -</span>
                    </div>
                </el-col>
          ...
          <el-input v-model="input" @change="discountData(input,2)"></el-input>
                ...
                <el-col :span="9">
                    <div class="grid-content bg-purple">{{$t('Software.details.discountedPrice')}}：
                        <span v-show="$store.state.userId !=1">
                            ￥<el-input v-model="input1" @change="getDiscountPrice(input1,2)" disabled></el-input>
                        </span>
                        <span v-show="$store.state.userId ==1">- -</span>
                    </div>
                </el-col>
...
export default {
  ...
  methods: {
    ...
    addDiscountData(type) {
      ...
      const discount = Number(this.input).toFixed(2);
      const discountPrice =this.input1.replace(/,/g, "");
            ...
            if (err) {
                ...
                for (let i = 0; i < res.length; i++) {
                    ...
                    if (res[i].purchase != undefined && res[i].purchase != null
                        ...
                        /*获取总价格*/
                        obj.total_price=res[i].purchase*obj.component_count
                        this.totalPrice+=res[i].purchase*obj.component_count
                    }
                    tempData.push(obj);
                }
                //处理hpo列表顺序
                    ...
                    if (err) {
                        ...
                        for (let j = 0; j < res.length; j++) {
                            ...
                            if (res[j].purchase != undefined && res[j].purchase != null
                                ...
                                /*获取总价格*/
                                objSWMA.total_price=res[j].purchase*objSWMA.component_count
                                this.totalPrice+=res[j].purchase*objSWMA.component_count
                            }
                       ...
                       // this.totalPrice = this.totalPrice.toFixed(2).replace(/\d{1,3}(?=(\d{3})+\b)/g, '$&,')

                        //let reg = this.totalPrice.toFixed(2)
                       //this.totalPrice = reg; ((this.totalPrice.replace(/,/g, ""))
