...
<script>
    export default {
        ...
        methods: {
            ...
            getRec(value, ind, index) {//点击选择框时的操作
                if (value) {//如果为true的时候为打开选择框，
                    ...
                    console.log(this.x16_4, this.x8_4, this.x8_3, blNum,this.brr)
      ...
      select info.name,detail.component_count,detail.categroy_id,io.PCI_E_generation,io.PCI_E_size from component_base_info info
                      join component_iocard io on info.id = io.base_info_id
                             join product_programme_detail detail on io.id=detail.component_id and  detail.categroy_id=${this.$route.query.sign}
      ...
      select info.name,detail.component_count,detail.categroy_id,info.description,mem.category_id from component_base_info info
                    ...
                    const updateSql = `INSERT INTO product_programme_detail('product_id','machine_id','categroy_id','component_id','component_count','seq_Key','solution_id','template_id','is_expansion') VALUES('${localStorage.productId}','${localStorage.machineId}','${this.$route.query.sign}','${index}','${num}','${time}','${localStorage.solutionId}','${localStorage.templateId}','1')`;
                    this.$db.run(updateSql, (err, res) => {
                        if (err) {
                            console.log('添加失败');
                        } else {
            ...
            getNum(ind, add) {//获取 this.x16_4、this.x8_4、this.x8_3 的值
                ...
                // }
                const SQL = `select PCI_E_generation,PCI_E_size ,component_count from component_iocard pp cross join product_programme_detail ppd on pp.id=ppd.component_id and ppd.categroy_id=${this.$route.query.sign} where ppd.component_id in(
      select ppd.component_id from product_programme_detail ppd where ppd.product_id='${localStorage.productId}'
      and ppd.categroy_id=${this.$route.query.sign} ) and template_id='${localStorage.templateId}'
 ...
 join component_iocard io on detail.categroy_id=io.category_id and detail.component_id=io.id
 join component_pcie_classification pcie on pcie.id=io.PCI_Catagory_id and pcie.id=3 where solution_id='${localStorage.solutionId}' and template_id='${localStorage.templateId}'and product_id='${localStorage.productId}' and machine_id='${localStorage.machineId}'`
                        ...
                        this.$db.get(getSQL, (err, res) => {
                            if (err) {
                                ...
                                if (res) {
                                    ...
                                    /*判断id不存在时添加*/
                                    const insert = `INSERT INTO product_programme_detail('product_id','machine_id','categroy_id','component_id','component_count','solution_id','template_id','is_expansion') VALUES('${localStorage.productId}','${localStorage.machineId}','17','5','${num}','${localStorage.solutionId}','${localStorage.templateId}','1')`;
            ...
            getEJ20() {
                // 获取库里ej20的数
                let getEj20 = `select detail.id,detail.component_id,detail.component_count,detail.is_expansion,info.description_en,
pcie.pcie_type from  product_programme_detail detail
 join component_iocard io on io.category_id=detail.categroy_id and io.id=detail.component_id
 ...
 where detail.component_id=35 and is_expansion in (1) and product_id='${localStorage.productId}'
                ...
                // 获取库里外部扩展柜内瓤的数
                let externalPcieSQL = `select detail.categroy_id,detail.component_id,detail.component_count from product_programme_detail detail
join component_expansionflesh flesh on flesh.categroy_id=detail.categroy_id and flesh.id=detail.component_id
where detail.is_expansion=0 and product_id='${localStorage.productId}' and machine_id='${localStorage.machineId}' and solution_id='${localStorage.solutionId}' and template_id='${localStorage.templateId}'`
                ...
                let hardDiskSQL=`select  detail.id,detail.component_count,detail.component_id,info.description description_en,pcie.pcie_type from product_programme_detail detail
join component_iocard io on io.category_id=detail.categroy_id and io.id=detail.component_id
join component_base_info info on info.id=io.base_info_id
join component_pcie_classification pcie on pcie.id=io.PCI_Catagory_id
...
and product_id='${localStorage.productId}' and machine_id='${localStorage.machineId}' and solution_id='${localStorage.solutionId}' and template_id='${localStorage.templateId}' `//and detail.is_expansion=1暂时不写
                /*ej0j的数量*/
                let ej0jSQL=`select detail.id,detail.component_count,mode.card_base_cnt from product_programme_detail detail
join component_disk_modeoption mode on mode.categroy_id=detail.categroy_id and mode.id=detail.component_id
where product_id='${localStorage.productId}' and machine_id='${localStorage.machineId}' and solution_id='${localStorage.solutionId}' and template_id='${localStorage.templateId}'`//暂时不写
                //这个是获取外部pcie的东西
                        ...
                        this.$db.get(getEj20, (err, res) => { // 获取库里ej20的数
                            ...
                            if (res) {
                                let obj = {
                                    ...
                                    description:[{type:res.description_en}],//选择展示的ej20的类型
                                    ...
                                    this.$db.get(hardDiskSQL, (err, res) => { // 获取库里ej20的数 hardDiskId:hardDiskNum:arrB
                                        ...
                                        if (res) {
                                            let obj = {
                                                ...
                                                pcie_type: 0,//ej0j的类型
                                                type:[{type:res.pcie_type}],//选择展示的类型
                                                description:[{type:res.description_en}],//选择展示的ej0j的类型
          ...
          WHERE PCI_Catagory_id =${item.PCI_Catagory_id}`;
            ...
            function run_b(arr) {//brr里面的值 和x8_3、x8_4、x16_4的值、还有展示的选择了多少
                return new Promise(function (resolve, reject) {
                    ...
                    const PCI_Catagory_id = `select info.name,detail.component_count,detail.categroy_id,io.PCI_E_generation,io.PCI_E_size from component_base_info info         join component_iocard io on info.id = io.base_info_id
                             join product_programme_detail detail on io.id=detail.component_id and
                              detail.categroy_id=${than.$route.query.sign}
                    ...
                    than.$db.all(PCI_Catagory_id, (err, res) => {
                        ...
                        if (than.cpu == 1) {
                            ...
                            if (x8_4 == 0) { //判断x8_4是不是为0  因为要向下兼容
                                if (x8_3 > 8) { //如果等于0的话就是x8_3为7个
                                    than.error = 'pcie内的x8_3内型选择过多';
                                    than.$emit('erro', 3);
                                    than.open();
                                }
                            } else {
                                if (x8_3 > 7) {//如果不等于0的话就是x8_3为6个
                                    than.error = 'pcie内的x8_3内型选择过多';
                                    than.$emit('erro', 3);
                                    than.open();
                                }
