...
import CfrConstants from './ipscsConstants';
...
const ipscsConvert = (csvStr, machineName,dataList) => {
  return new Promise((resolve, reject) => {
    strConvertCfr(csvStr, machineName, true,dataList);
  });
};
...
function strConvertCfr(csvStr, machineName, flag,dataList) {
  let listOfRecoders = [];
  ...
  let isSort = true;
  sCfr = createCfr(listOfRecoders, listOfHistory, listOfProposedRecoders, isSort,dataList);
  if (flag) {
...
function createCfr(listOfRecords, listOfBaseRecords, listOfProposedRecords, isSort,dataList) {
  let i = 0;
  let cfrList = [];
  let isHeader = false;
  ...
  let arRecords05 = createRecord05('',dataList);
  ...
  if (!isEmpty(sCRAD)) {
    arRecords05 = createRecord05(sCRAD,dataList);
  ...
  if (isSort) {
    ...
    while (j < listOfRecords.length) {
      ...
      while (j < listOfRecords.length) {
        ...
        if (CfrConstants.CfrConstants.LINE_PRODUCT_TYPE_HARDWARE == sProductType) {
          //key拼接 is_expansion和powerline_type
          mapHardware.set(sMTM + sSign + arFields[CfrConstants.CfrConstants.LINE_FEATURE]+
              arFields[23] + arFields[24] , arFields);
        } else if (CfrConstants.CfrConstants.LINE_PRODUCT_TYPE_SOFTWARE == sProductType) {
          //key拼接 is_expansion和powerline_type
          mapSoftware.set(sMTM + sSign + arFields[CfrConstants.CfrConstants.LINE_FEATURE]+
              arFields[23] + arFields[24], arFields);
        } else if ((CfrConstants.CfrConstants.LINE_PRODUCT_TYPE_SERVICES == sProductType) || (CfrConstants.CfrConstants.LINE_PRODUCT_TYPE_SERVICEPAC == sProductType)) {
          //key拼接 is_expansion和powerline_type
          mapServices.set(sMTM + sSign + arFields[CfrConstants.CfrConstants.LINE_FEATURE]+
              arFields[23] + arFields[24], arFields);
        }
        j++;
      } // ------ subline
    ...
    hardwareObj.sort(function(a, b) {
      return a[0].localeCompare(b[0]);
    });
    //软件map排序
    let softwareObj = Array.from(mapSoftware);
    softwareObj.sort(function(a, b) {
      return a[0].localeCompare(b[0]);
    });
    //服务map排序
    let servicesObj = Array.from(mapServices);
    servicesObj.sort(function(a, b) {
      return a[0].localeCompare(b[0]);
    });
...
function createRecord05(sCRAD,dataList) {
  let arRecords ;
  if (dataList.length > 0){
    arRecords = new Array(dataList.length + 8);
  }else{
    arRecords = new Array(5);
  }
  ...
  sBuf = setRecordField(sBuf, '0', 13, 13, false);
  sBuf = setRecordField(sBuf, 'Created on ' + moment()
      .format('MMMM ') + moment()
      .format('DD') + ', '
    + moment()
      .format('YYYY ') + 'at' + moment()
      .format(' hh:mmA') +
  ...
  if (dataList.length > 0){
    ...
    sBuf = setRecordField(sBuf, '05', 1, 2, false);
    sBuf = setRecordField(sBuf, '1', 12, 12, false);
    sBuf = setRecordField(sBuf, 'U', 13, 13, false);
    sBuf = setRecordField(sBuf,"You selected shipping date is : "+ sTmp,17, 76, false);
    arRecords[3] = sBuf;
    //0U
    sBuf = '';
    sBuf = createRecord(sBuf, 76);
    ...
    sBuf = setRecordField(sBuf, '0', 12, 12, false);
    sBuf = setRecordField(sBuf, 'U', 13, 13, false);
    sBuf = setRecordField(sBuf,"The following product/features are withdraw with",17, 76, false);
    arRecords[4] = sBuf;
    ...
    sBuf = setRecordField(sBuf, '0', 12, 12, false);
    sBuf = setRecordField(sBuf, 'U', 13, 13, false);
    sBuf = setRecordField(sBuf,"respect to the selected Customer Requested Arrival Date :",17, 76, false);
    arRecords[5] = sBuf;
    //循环客户要求日期大于退市日期
    for (let i = 0; i < dataList.length; i++) {
      ...
      sBuf = setRecordField(sBuf, '0', 12, 12, false);
      sBuf = setRecordField(sBuf, 'U', 13, 13, false);
      sBuf = setRecordField(sBuf,dataList[i].machineNumber+' '+dataList[i].featureCode
          +' '+ moment(dataList[i].withdrawDate).format('YYYYMMDD'),
          17, 76, false);
      arRecords[i+6] = sBuf;
    }
    sBuf = '';
    ...
    arRecords[dataList.length+6] = sBuf;
    ...
    sBuf = setRecordField(sBuf, '0', 13, 13, false);
    sBuf = setRecordField(sBuf, '000', 14, 16, false);
    arRecords[dataList.length+7] = sBuf;
  }else {
    sBuf = '';
    sBuf = createRecord(sBuf, 76);
    sBuf = setRecordField(sBuf, '05', 1, 2, false);
...
function createRecord96(arFields) {
  ...
  //------------------------------------------------
  //设置is_expansion 和powerline_type 以及 是否导出hpo的标识 0：不导出 1：导出
  sTmp = arFields[23]; //is_expansion
  strBuf = setRecordField(strBuf, sTmp, 30, 30, false);

  sTmp = arFields[24]; //powerline_type
  strBuf = setRecordField(strBuf, sTmp, 31, 31, false);
  sTmp = arFields[25]; //powerline_type
  strBuf = setRecordField(strBuf, sTmp, 32, 32, false);
  //机器数量
  sTmp = arFields[26]; //powerline_type
  strBuf = setRecordField(strBuf, sTmp, 33, 34, false);
  //是否用户手动修改/添加标识
  if (isEmpty(arFields[27])){
    sTmp = '';
  }else{
    sTmp = arFields[27]; //addition_flag
  }
  strBuf = setRecordField(strBuf, sTmp, 35, 35, false);
  //拓展柜id
  if (isEmpty(arFields[28])){
    sTmp = '';
  }else{
    sTmp = arFields[28]; //addition_flag
  }
  strBuf = setRecordField(strBuf, sTmp, 36, 40, false);
  //------------------------------------------------
...
function createRecord07(arFields) {
  ...
  //设置templateId  导入时使用
  sBuf = setRecordField(sBuf, arFields[22], 78, 82, false);
  return sBuf;
}
...
function createRecord00(arFields) {
  ...
  // PWR: Power (brand)
  sBuf = setRecordField(sBuf, 'CHNIPS', 13, 20, false);
...
function computeChecksum(sRecord, lCheckSum) {
  ...
  for (let i = 0; i < len; i++) {
    lineRecordByte[i] = sRecord.charAt(i)
      .charCodeAt(0);
  }
...
export default {
  ipscsConvert, createCfr, getErrMsg, strConvertCfr
};
