...
function parseRecord07(sRecord) {
        ...
        //解析模板id
        arRecords[6] = getElement(sRecord, 78, 82);
...
function parseRecord96(sRecord) {
        ...
        //------------------------------------------------------
        //解析is_expansion 和powerline_type 以及是否导出hpo标识
        arRecords[23] = getElement(sRecord, 30, 30);
        arRecords[24] = getElement(sRecord, 31, 31);
        arRecords[25] = getElement(sRecord, 32, 32);
        //机器数量
        arRecords[26] = getElement(sRecord, 33, 34);
        //是否用户手动修改/添加标识
        arRecords[27] = getElement(sRecord, 35, 35);
        if (arRecords[27].length == 0) {
            arRecords[27] = '';
        }
        //拓展柜id标识
        arRecords[28] = getElement(sRecord, 36, 40);
        if (arRecords[28].length == 0) {
            arRecords[28] = '';
        }
        //------------------------------------------------
