...
<script>
    export default {
        ...
        props: ['externalSum', 'sign','pcieSwitch'],
        /*自定义事件*/
        ...
        methods: {
            ...
            addPcie(bool) {
                ...
                this.arr.push({
                    ...
                    detailsType: this.childType,//详情的数组
                    num: bool?'':1,//数量
                    numZ: '',//要展示的总数量
                    id: '', //所选中的id
                    typeId: 1,//类型的id
            ...
            /*获取库里外部pcie的内容*/
            getPcie() {
                const defaltuPci = `select pp.PCI_E_generation,PCI_E_size,PCI_Catagory_id,info.description PCI_E_desc_cn,info.description_en,ppd.component_count,ppd.id,ppd.seq_Key,ppd.component_id,info.status,info.withdraw_date from component_iocard pp
                ...
                and machine_id='${localStorage.machineId}' and is_expansion='0'`;// 获取库里面的值外部pcie的值、id、代数等..  ，
                this.$db.all(defaltuPci, (err, res) => {
                    if (err) {
                        ...
                        res.forEach((item, index) => {
                            ...
                            this.getDetails(item.PCI_Catagory_id, index)
                        })
                        console.log(res, this.arr, this.x16_3, this.x8_3,this.externalPcie,this.hardDiskId)
            ...
            /*根据pcie类型来详情的内容、类型的id以及下标*/
            getDetails(id, index, bool) {
                console.log(2736463,this.externalPcie)
                let getDetail = `SELECT io.*,info.description PCI_E_desc_cn,info.description_en,info.status,info.withdraw_date FROM COMPONENT_IOCARD io left join component_base_info info on info.id=io.base_info_id WHERE PCI_Catagory_id =${id} and PCI_E_generation in (1,2,3) and info.productId='${localStorage.productId}'`;
                    ...
                    if (err) {
                        ...
                        if (!bool) {
                            ...
                            if (res.length) {
                                ...
                                if (this.x16_3 >= this.externalSum * 2) {
                                    arr = arr.filter(item => {
            ...
            setList(bool, index) {
                if (bool) {
                    ...
                    if (this.arr[index].PCI_E_size == 1) {
                        this.arr[index].numZ = this.externalSum * 6 - this.x8_3 - this.x16_3 + this.arr[index].num
                    } else if (this.arr[index].PCI_E_size == 0) {
                        this.arr[index].numZ = 1
                    } else {
                        this.arr[index].numZ = this.externalSum * 2 - this.x16_3 + this.arr[index].num
                    }
                }
            },
            /*点击后要展示的内容*/
 ...
 join component_pcie_classification pcie on pcie.id=io.PCI_Catagory_id and pcie.id=3 where solution_id='${localStorage.solutionId}' and template_id='${localStorage.templateId}'and product_id='${localStorage.productId}' and machine_id='${localStorage.machineId}'`
            ...
            getEJ20() {
                ...
                let hardDiskSQL = `select  detail.id,detail.component_count,detail.component_id,PCI_E_size,info.description description_en,pcie.pcie_type,info.status,info.withdraw_date,info.title_msg,info.ui_status from product_programme_detail detail
...
where product_id='${localStorage.productId}' and machine_id='${localStorage.machineId}' and solution_id='${localStorage.solutionId}' and template_id='${localStorage.templateId}' and detail.is_expansion=0`//and detail.is_expansion=0暂时不写
                ...
                this.$db.all(ej0jSQL, (err, res) => { // 获取库里外部扩展柜内瓤的数
                    if (res.length) {
                        ...
                        console.log(res,num)
                        this.$db.get(hardDiskSQL, (err, res) => { // 获取库里ej0j的数
                            ...
                            if (res) {
                                let obj = {
                                    ...
                                    num: res.component_count,//记录下要展示的数量，减去x8_3的数量时用
                                    externalSum: [],//ej0j的展开的数量
                                    PCI_E_size:res.PCI_E_size//展示那个类型，比如x8_3
                                }
        ...
        created() {
            ...
            let getDetails = `SELECT io.*,info.description PCI_E_desc_cn,info.description_en,info.status,info.withdraw_date FROM COMPONENT_IOCARD io left join component_base_info info on info.id=io.base_info_id WHERE PCI_Catagory_id =1 and PCI_E_generation in (1,2,3) and info.productId= '${localStorage.productId}'`; //获取Lan的参数
