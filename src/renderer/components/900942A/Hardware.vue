            ...
            <el-row :gutter="24" v-show="!hardwareBian">
                <el-col :span="7">
                    <div class="grid-content grid-total">{{$t('Software.details.aggregate')}}：
                        <span v-show="$store.state.userId !=1" >{{totalPrice | totalPriceFilters}}</span>
                        <span v-show="$store.state.userId ==1">- -</span>
                    </div>
                </el-col>
                <el-col :span="8">
                ...
                <el-col :span="9">
                    <div class="grid-content bg-purple">{{$t('Software.details.discountedPrice')}}：
                        <span v-show="$store.state.userId !=1">
                            ￥<el-input v-model="input1" @change="getDiscountPrice(input1,1)" disabled></el-input>
                        </span>
                        <span v-show="$store.state.userId ==1">- -</span>
                    </div>
                </el-col>
    ...
    export default {
        ...
        data() {
            return {
                ...
                config: [//硬件配置的初始显示
                    ...
                    {
                        name: '配件',
                        img: __static + '/accessories-1.png',
                        boole: true,
                        erre: false,
                        id:5
                    },
        ...
        methods: {
            ...
            discountData(num,type) {
                this.input = Number(num).toFixed(2)
                if (this.input < 100 && this.input >=0) {
                    this.input1 = ((this.totalPrice*(100-this.input) / 100).toFixed(2)).replace(/\d{1,3}(?=(\d{3})+\b)/g, '$&,')
            ...
            addDiscountData(type) {
                ...
                let totalPrice = parseInt(this.totalPrice);
                let discount = this.input;
                let discountPrice =parseInt((this.input1).replace(/,/g, ""));
...
join component_disk disk on disk.category_id=detail.category_id and disk.id=detail.component_id and solution_id='${localStorage.solutionId}' and product_id='${localStorage.productId}'
            and template_id='${localStorage.templateId}' and machine_id='${localStorage.machineId}'`;
                ...
                this.$db.all(SQL, (err, res) => {
                        if (err) {
                            ...
                            if (index == 1) {
                                this.$router.push({path: '/Hardware/Memory', query: {id: 1,sign:this.config[index].id,signT:this.config[index].id1,otheId:this.config[index].otheId}});
                            }
                            if (index == 7) {
                            ...
                            if (index == 2) {
                                ...
                                this.$router.push({path: '/Hardware/InsideHardDiskDrive', query: {id: 2,showMax:false,sign:this.config[index].id,signT:this.config[index].id1,nameRu:this.config[index].nameRu}});

                            }

                            if (index == 4) {
                                /*跳转 添加showMax 来 显示 总容量*/
                                this.$router.push({path: '/Hardware/ExternalHardDisk', query: {id: 4,showMax:false,sign:this.config[index].id,signT:this.config[index].id1,xpansionId:this.config[index].xpansionId,modelId:this.config[index].modelId}});
            ...
            reload() {
                ...
                console.log(id)
                this.$router.push({path: '/SelectTemplate', query: {id:id}});
            },
        ...
        created() {
            ...
            const update = `select id,configure_type,discount from machine_price_detail where machine_id=${machineId}`
