...
async function getDataListHW(productId, solutionId, templateId, machineId) {
  ...
  const SQL = `SELECT
                ...
                info.listprice_onshore,info.listprice_offshore,so.ship_date,pi.machine_model,pi.machine_submodel,  pi.description AS pdesc,  info.withdraw_date,info.ui_status     
               ...
               UNION  ALL
               SELECT
               detail.id,1 AS component_PN,1 AS component_FC,null, soft.software_desCN, soft.software_des,detail.component_count,detail.category_id,detail.component_id,
               soft.purchase,null,null,null,null,  null,  null,soft.ui_status   
               FROM product_programme_detail detail 
  ...
  for (let i = dataListHW.length - 1; i >= 0; i--) {
    for (let j = additionList.length - 1; j >= 0; j--) {
      ...
      if (dataListHW[i].id == additionList[j].origin_fc_id) {
        ...
        dataListHW[i].listprice_offshore = '';
...
//line 19------------------------------------------------------------------------------------<
  // Excel row19 待完善 该行显示的是machineIds所使用的产品的信息
  //机型
...
//------------------------------------------------------------------------------------------------------------>
  ...
  for (let i = 0; i < dataListHW.length + 1; i++) {
    ...
    if (flagHW) {
      ...
      discount = dfu.numDealUnToFixed(discount) * 1.0;
  ...
  if (hpoExportFlag) {
    ...
    for (let o = 0; o < additionListSW.length; o++) {
      ...
      // FeatureCode
      let componentFC = cusItem.software_FC;

      // 数量
      let componentCount = cusItem.component_count;

      // 描述
      let description = cusItem.software_des;
      description = description == null ? '' : description;
      ...
      // 软硬件类型 1=纯硬件MTM，2=HPO，3=软件，4=服务
      let componentTypeSeri = cusItem.category_id == 35 ? 4 : (cusItem.category_id == 34 ? 2 : 3);
  ...
  for (let o = headerSW.length - 1; o >= 0; o--) {
    if (headerSW[o].Classify.substring(5)
      .toUpperCase() == 'A6P' && headerSW[o].software_category == 1) {
      operList.push(headerSW[o]);
      headerSW.splice(o, 1);
      continue;
    }
    if (headerSW[o].Classify.substring(5)
      .toUpperCase() == 'A6P' && headerSW[o].software_category == 0) {
      logicList.push(headerSW[o]);
      headerSW.splice(o, 1);
      continue;
    }
    if (headerSW[o].Classify.substring(5)
      .toUpperCase() == 'A6P' && headerSW[o].software_category == 2) {
      applicationList.push(headerSW[o]);
      headerSW.splice(o, 1);
      continue;
    }
  }
  headerSW = operList.concat(logicList)
    ...
    for (let j = 0; j < dataListSW.length; j++) {
      ...
      if (headerSW[m].Classify == dataListSW[j].Classify && headerSW[m].software_category == dataListSW[j].software_category) {
        ...
        if (flagSW) {
          ...
          componentCount = machinePropertySW1.QUANTITY;
          ...
          // 自定义折后价（含税）
          customerdisCountListPriceAfterTax;
          ...
          // 自定义折后价（含税）
          let customerdisCountListPriceAfterTax;
          customerdisCountListPriceAfterTax = (eval(cusDiscountPriceUnTax + '*' + '(1+' + tax + ')'));
          customerdisCountListPriceAfterTax = dfu.numDeal(customerdisCountListPriceAfterTax);
    ...
    for (let o = 0; o < swmaList.length; o++) {
      ...
      if (headerSWMA[m].Classify == swmaList[o].Classify) {
        if (flagSWMA) {
          ...
          componentCount = swmaList[o].component_count;
          ...
          //列表价未税
          let listPriceUntax = isRMB ? itemswma.purchase : itemswma.USDPurchase;
          listPriceUntax = dfu.numDeal(listPriceUntax);
          listPriceUntax = dfu.numDeal(eval(listPriceUntax + '*' + componentCount));
