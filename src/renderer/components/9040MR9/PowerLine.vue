...
<script>
    export default {
     ...
     join component_power power on power.base_info_id=info.id and power.base_info_id=643`; // 电源
                const SQLNUM = 'SELECT info.id,info.description,info.description_en line_des,info.status,info.withdraw_date,info.title_msg,info.ui_status \n' +
                    'FROM component_powerline line \n' +
                    'join component_base_info info on info.id=line.base_info_id\n' +
                    'where info.productId=2'; // 电源线
                // let obj = {};
